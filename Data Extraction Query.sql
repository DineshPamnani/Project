...
SELECT COUNT(DISTINCT(Epno)) Total_Episode FROM sharktank_data;

-- 3) Total Pitches 
SELECT COUNT(DISTINCT(Brand)) Total_Pitches FROM sharktank_data;
...
-- 5) Successful investment count
SELECT (count(Amount_Invested) / (SELECT count(Brand) FROM sharktank_data)) Successful_Investment_Count
FROM sharktank_data 
WHERE Amount_Invested > 0;

-- 6) Total Male pitchers
SELECT Sum(Male) Male_Pitchers FROM sharktank_data;

-- 7) Total Female pitchers
SELECT Sum(Female) Female_Pitchers FROM sharktank_data;
...
SELECT max(Amount_Invested) Highest_Investment_In_Lakhs FROM sharktank_data;

-- 12) Highest Equity Taken
SELECT max(Equity_Taken_Percentage) Highest_Equity_Taken FROM sharktank_data;

-- 13) Pitches with atleast 1 Female contestant
...
SELECT count(s.female) Successful_Pitch_with_atleast_1_Female_Contestant FROM (
SELECT * FROM sharktank_data WHERE Amount_Invested > 0) s WHERE Female > 0;

-- 15) Avg Team members
SELECT avg(Team_members) Avg_Team_Size FROM sharktank_data;

-- 16) Avg Amount invested per deal (only where funding received)
...
-- Main Query (Anupam)
SELECT p.keyy, p.Anupam_Present,p.Anupam_Invested_Count,  q.Total_Amount_Invested,Round(q.Average_Amount_Invested,2) FROM (
